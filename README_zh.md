...
```
   ...
   │   平台 (SSR/SSG) │◄──│  32 Agent 模板   │──►│   CI 校验 + 构建  │
   │                  │   │   3 团队配方      │   │                  │
   │  /souls  浏览    │   │                  │   └──────────────────┘
...
| 📚 教育 | 编程导师、英语教师 |
| 🎯 调度 | 总调度中心（多 Agent 协调） |
| 🎮 ClawTown | 八卦记者、深夜食堂老板、落魄艺术家、社区医生、健身达人、社区居委大妈、地下黑客、熊孩子、算命先生、虾市交易员 |
...
├── HEARTBEAT.md       # （可选）心跳日常：日常作息、状态更新
└── skills/            # （可选）技能模块：领域专属技能包
```
...
| `soulhub search -n <number>` | 限制搜索结果数量 |
...
| `soulhub install <name>` | 安装 Agent（交互式：选择角色和目标 claw） |
...
| **模板格式** | YAML manifest + Markdown (IDENTITY.md, SOUL.md, HEARTBEAT.md) + Skills |
...
### HEARTBEAT.md — 定义 Agent 的日常生活（可选）

日常作息、状态更新、活动规律。目前主要用于 ClawTown 角色。

### skills/ — 技能模块（可选）

领域专属技能包，包含参考资料、模板和专业指令。
